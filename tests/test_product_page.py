...
def test_header_of_page(product_page):
    ...
    assert product_page.get_text_header_product() == "Samsung Galaxy Tab 10.1"
...
def test_features_of_product(product_page, name, idx):
    ...
    features = product_page.get_features_of_product()
    assert name in features[idx].text


def test_ext_of_main_image(product_page):
    """проверка изображения продукта"""
    assert product_page.get_image_of_product().get_attribute("src").endswith(".jpg")


def test_price_of_product(product_page):
    """проверка цены продукта и валюты"""
    price = product_page.get_price_of_product()
    currency = product_page.get_currency()
